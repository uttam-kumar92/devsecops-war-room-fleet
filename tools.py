        ...
        with _http_session_lock:
            if _http_session is None:
                ...
                retry_strategy = Retry(
                    total=2, backoff_factor=0.3, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["GET"]
                )
...
class SecurityRuleRegistry:
    ...
    def __init__(self, rule_file_path: str | None = None):
    ...
    def _load_rules(self) -> None:
        ...
        if os.path.exists(self.rule_file_path):
            try:
                with open(self.rule_file_path, encoding="utf-8") as f:
                    data = json.load(f)
                    ...
                    if "known_secret_patterns" in data and isinstance(data["known_secret_patterns"], list):
                        ...
                        for item in data["known_secret_patterns"]:
                            if isinstance(item, dict):
                                patterns.append(
                                    (item.get("name", "Secret"), item.get("pattern", ""), item.get("severity", "High"))
                                )
...
def scan_high_entropy_secrets(code: str, min_length: int = 14, entropy_threshold: float = 3.8) -> list[dict[str, Any]]:
    ...
    for match in re.finditer(
        r"(?:KEY|SECRET|TOKEN|PASSWORD|PASSWD|AUTH|CREDENTIAL)\s*[:=]\s*([a-zA-Z0-9_\-\.+=/!@#$%^&*]{12,})",
        code,
        re.IGNORECASE,
    ):
        token = match.group(1)
...
class PythonASTSecurityScanner(ast.NodeVisitor):
        ...
        if isinstance(node, ast.Name):
            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr == "format":
                if self._is_tainted(node.func.value):
                    return True
                return any(self._is_tainted(arg) for arg in node.args) or any(
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        is_route_handler = any(
            ...
            and any(
                r in self._get_node_name(d).lower()
                for r in ("route", "get", "post", "put", "delete", "api", "endpoint")
            )
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if func_name in (
            ...
            has_safe_loader = any(
                kw.arg == "Loader" and "SafeLoader" in getattr(kw.value, "id", "") for kw in node.keywords
            )
        ...
        if attr_name == "run" or func_name in ("app.run", "server.run"):
            has_all_interfaces = any(
                isinstance(kw.value, ast.Constant) and kw.value.value == "0.0.0.0"
                for kw in node.keywords
                if kw.arg == "host"
            )
            has_debug_true = any(
        ...
        has_verify_false = any(
            isinstance(kw.value, ast.Constant) and kw.value.value is False for kw in node.keywords if kw.arg == "verify"
        )
        ...
        if (
            attr_name in ("fromstring", "parse")
            and any(k in func_name.lower() for k in ("et", "xml", "tree", "sax", "minidom", "lxml"))
        ) or func_name in (
        ...
        if func_name in ("jwt.decode", "jose.jwt.decode"):
            has_unverified = any(
                ...
                or (
                    kw.arg == "options"
                    and "verify_signature" in self._get_snippet(kw.value)
                    and "false" in self._get_snippet(kw.value).lower()
                )
                for kw in node.keywords
            )
            has_none_algo = any(
                kw.arg == "algorithms" and "none" in self._get_snippet(kw.value).lower() for kw in node.keywords
            )
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if any(
                    secret_term in var_name
                    for secret_term in ("secret", "token", "password", "api_key", "jwt", "private_key", "aws_key")
                ):
...
def generate_unified_diff(original_code: str, patched_code: str, filename: str = "app.py") -> str:
    ...
    diff = difflib.unified_diff(
        orig_lines, patch_lines, fromfile=f"a/{filename} (vulnerable)", tofile=f"b/{filename} (hardened)", lineterm=""
    )
    return "".join(diff)
                ...
                "locations": [
                    {
                        "physicalLocation": {
                            ...
                            "region": {"startLine": vuln.get("line", 1), "snippet": {"text": vuln.get("snippet", "")}},
                        }
                    }
                ],
            }
            ...
            rules.append(
                {
                    ...
                    "properties": {"tags": ["security", "CWE-798", "secrets"]},
        ...
        results.append(
            {
                ...
                "level": "error",
                "message": {
                    "text": f"Exposed high-entropy secret token ({secret.get('entropy')} bits): {secret.get('masked_token')}"
                },
                "locations": [
                    {
                        "physicalLocation": {
...
def fetch_osv_vulnerabilities(
    package_name: str, version: str | None = None, ecosystem: str = "PyPI", timeout: int = 3
) -> list[dict[str, Any]]:
    """
                ...
                results.append(
                    {
                        ...
                        "details": (v.get("details", "")[:250] + "...")
                        if len(v.get("details", "")) > 250
                        else v.get("details", ""),
...
def fetch_cve_threat_intel(query: str, timeout: int = 3) -> str:
    ...
    def _fetch_ddg_api() -> str | None:
        try:
            url = (
                f"https://api.duckduckgo.com/?q={encoded_query}+vulnerability+cve&format=json&no_html=1&skip_disambig=1"
            )
    ...
    def _fetch_osv() -> str | None:
        ...
        for word in words:
            ...
            if clean_word in ECOSYSTEM_PACKAGE_MAP:
                ...
                if osv_records:
                    rec_lines = [
                        f"- **[{r['id']}]** ({', '.join(r['aliases']) if r['aliases'] else 'GHSA/CVE'} | {r.get('ecosystem', eco)}): {r['summary']}"
                        for r in osv_records[:3]
                    ]
...
def generate_security_dashboard_figures(
    stride_scores: Any | None = None, cvss_score: float = 8.5, heuristic_metrics: dict[str, Any] | None = None
) -> dict[str, go.Figure]:
    """
    ...
    fig_gauge = go.Figure(
        go.Indicator(
            ...
            domain={"x": [0, 1], "y": [0, 1]},
            title={
                "text": "CVSS 3.1 BASE SCORE",
                "font": {"color": "#888888", "size": 12, "family": "JetBrains Mono, monospace"},
            },
            ...
            gauge={
                ...
                "threshold": {"line": {"color": "#ffffff", "width": 2}, "thickness": 0.75, "value": cvss_val},
            },
        )
    )
    ...
    return {"stride_radar": fig_stride, "cvss_gauge": fig_gauge, "risk_breakdown": fig_bar}
